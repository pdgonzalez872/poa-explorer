  ...
  *Note*: The internal transactions for individual transactions cannot be paginated,
  ...
  def run(transaction_hashes, _retries) do
    ...
    case EthereumJSONRPC.fetch_internal_transactions(transaction_hashes) do
      {:ok, internal_params} ->
        transaction_hashes
        |> import_internal_transactions(internal_params)
        |> fetch_new_balances()

      {:error, reason} ->
        Indexer.debug(fn -> "failed to fetch internal transactions for #{length(transaction_hashes)} transactions: #{inspect reason}" end)
        {:retry, reason}
    end
  end

  defp import_internal_transactions(transaction_hashes, internal_params) do
    Chain.import_internal_transactions(transaction_hashes, internal_params)
  end
