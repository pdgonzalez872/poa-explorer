defmodule Explorer.SmartContract.Solidity.CodeCompiler do
  @moduledoc """
  Module responsibile to compile the Solidity code of a given Smart Contract.
  """
  ...
  ## Examples
      iex(1)> Explorer.SmartContract.Solidity.CodeCompiler.run("SimpleStorage", "pragma solidity ^0.4.23; contract SimpleStorage {uint storedData; function set(uint x) public {storedData = x; } function get() public constant returns (uint) {return storedData; } }")
      %{
        "contracts" => %{
          "SimpleStorage" => %{
            "SimpleStorage" => %{
  ...
  """
  def run(name, code) do
    {response, _status} =
      System.cmd(
        Application.app_dir(:explorer, "priv/solc.bash"),
        [generate_settings(name, code)]
      )
  ...
  """
  def generate_settings(name, code) do
    """
    {
      "language": "Solidity",
      "sources": {
